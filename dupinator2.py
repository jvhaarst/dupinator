...
def walker(arg, dirname, fnames):
    ...
    try:
        ...
        pass
    for f in fnames:
        if not os.path.isfile(f) or os.path.islink(f) or f == '.DS_Store' or string.lower(".svn") in string.lower(f) or string.lower("PSG10074_D.backup") in string.lower(f) or string.lower("kwatta_database") in string.lower(f):
            continue
